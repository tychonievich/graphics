...
By combining multiple wavelengths we can get any point inside that curve. Points outside the curve represent cone responses that cannot be triggered by any combination of light.^[In 2025, researchers caused the optic nerve to signal the impossible ratio $(L,M,S)=(0,1,0)$ by targeting microscopic lasers directly at M cones and only M cones. The subjects report seeing an "impossibly saturate" color that was sort of bluish-green, which they named "[Olo](https://en.wikipedia.org/wiki/Olo_(color))".]
