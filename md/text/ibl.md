...
If all objects were perfectly smooth, all we'd need for specular lighting
would be an easier-to-look-up representation of the environment map itself.
[Octahedral maps](math.html#octahedral-maps) are one of the best choices for that representation,
though [cube maps](math.html#dube-maps) remain popular as well.

To use this map for smooth objects, we reflect the view direction across the surface normal
and look up the resulting direction in the octahedral or cube map;
the result is the total specular reflection color at that point.

But very few objects are perfectly smooth:
most are rough, with varying levels of roughness.
To handle roughness we need both a model of how normals are distributed on a rough object
and a way of computing the resulting shine.

The most popular model of roughness normals is usually called GGX,
based on the assumption that surface normals are distributed as if the object was covered in oblate spheroids,
but that basis is mostly irrelevant to its computation
which has is expensive enough to compute directly
that it is mostly handled via polynomial approximations.

<details class="aside"><summary>Attribution and naming of GGX and its approximation</summary>

The GGX distribution was first published in
...
It was not used in graphics.

A set of polynomial approximations for a different specular function was published in
...
The GGX distribution was re-derived and given that name in
...
While this derivation was used in graphics, it was too computationally expensive for most applications.

The Walter <i lang="la">et al</i> paper does not explain why they called it "GGX",
but since GGX was derived to model various ground glass materials
it is generally assumed to be derived from "Ground Glass X"
where the X represents the variable levels of grinding.

Schlick's polynomials were adjusted to fit GGX
by Brian Karis as he was working on Unreal Engine 4,
with the results shared in

> Brian Karis. 2013. "Real Shading in Unreal Engine 4." *SIGGRAPH Course: Physically Based Shading in Theory and Practice*, vol. 4, Epic Games.

Since then, "Schlick GGX" has become the common name for code implementing this,
while "Towbridge-Reitz" is starting to gain over "GGX" for the name of the exact distribution.

</details>

Unfortunately for IBL, the fast polynomial version of GGX assumes analytical light directions.
While it is possible to sample every pixel in the input environment map
at every view direction in the output octahedral map,
doing so is prohibitively expensive.
Instead, we make use of the underlying spheroid definition to randomly sample multiple likely reflectance directions,
approximating the full GGX illumination in a Monte Carlo integration.

:::example
The following is WGSL code that shows the code of Monte Carlo GGX sampling

```WGSL
// Generates a sample direction centered around a surface normal
fn importanceSampleGGX(randomVal: vec2<f32>, N: vec3<f32>, roughness: f32) -> vec3<f32> {
    const PI = 3.14159265359;
    let alpha = roughness * roughness;
    let alpha2 = alpha * alpha;

    // 1. Compute the spherical angles for the microfacet normal (H)
    let phi = 2.0 * PI * randomVal.x;
    
    let cosTheta = sqrt((1.0 - randomVal.y) / (1.0 + (alpha2 - 1.0) * randomVal.y));
    let sinTheta = sqrt(max(0.0, 1.0 - cosTheta * cosTheta));

    // 2. Map spherical angles to a 3D Cartesian vector in local space
    ...
    // 3. Transform H from local space to world space aligned with normal N
    let up = select(vec3<f32>(1.0, 0.0, 0.0), vec3<f32>(0.0, 0.0, 1.0), abs(N.z) < 0.999);
    let tangent = normalize(cross(up, N));
    let bitangent = cross(N, tangent);
    
    let H_world = normalize(tangent * H_local.x + bitangent * H_local.y + N * H_local.z);
