...
const lbez = (t, ...p) => {
  while(p.length > 1) p = p.slice(1).map((i,e) => lerp(t,p[i],e))
  return p[0]
}
const bezcut = (t, ...p) => {
  let front = [], back = []
  while(p.length > 0) {
    ...
    p = p.slice(1).map((i,e) => lerp(t,p[i],e))
  }
  return [front, back]
}
const slerp = (t,q0,q1) => {
...
const sbez = (t, ...p) => {
  while(p.length > 1) p = p.slice(1).map((i,e) => slerp(t,p[i],e))
  return p[0]
}
const qbez = (t, ...p) => {
  while(p.length > 1) p = p.slice(1).map((i,e) => qlerp(t,p[i],e))
  return p[0]
}
